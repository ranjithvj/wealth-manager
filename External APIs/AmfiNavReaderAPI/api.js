...
router.get('/getbyschemecode', function (req, res) {
  const url = 'https://www.amfiindia.com/spages/NAVAll.txt'
  ...
  request(url, function (error, response, body) {
    ...
    fundData = fundData.filter((fund) => {
      if (fund['Scheme Code'] == req.query.schemeCode)
        return fund
    })
...
router.get('/getbyisin', function (req, res) {
  const url = 'https://www.amfiindia.com/spages/NAVAll.txt'
  ...
  request(url, function (error, response, body) {
    ...
    fundData = fundData.filter((fund) => {
      if (fund['ISIN Div Payout/ ISIN Growth'] == req.query.isin)
        return fund
    })
